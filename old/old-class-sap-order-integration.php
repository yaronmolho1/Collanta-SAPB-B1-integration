...
function sap_handle_order_integration($order_id) {
    ...
    if ($existing_customer_code) {
        ...
        "SalesPersonCode" => 1, // Changed to match your fields_orderAPIi.json
    ...
    if ($sap_customer_code) {
        ...
        if ($credit_card_data) {
            $payment_data_for_sap = [
                ...
                "Series"        => 15, // Verify this Series exists for incoming payments
            ...
            error_log('SAP Integration: Payment data configured - Series: 15, Customer: ' . $sap_customer_code . ', OrderRef: ' . $order->get_order_number());
        ...
        $order_flow_payload = [
            "Order" => [
                ...
                "Comments"        => "WooCommerce Order #" . $order->get_order_number() . " from " . $customer_email,
                "JournalMemo"     => "Sales Orders - " . $sap_customer_code,
                "SalesPersonCode" => 2,
                "Series"          => 8, // Changed to match your fields_orderAPIi.json
                "DocumentsOwner"  => 1,
                "DocumentLines"   => $order_lines_for_sap,
                "U_ZipCode"       => $shipping_postcode,
                "U_shipToStreet1" => $shipping_address_1,
                "U_shipToStreet2" => $shipping_address_2,
                "U_City"          => $shipping_city,
